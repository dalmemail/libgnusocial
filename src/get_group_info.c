...
gnusocial_group_info_t gs_get_group_info(gnusocial_account_t account, int id)
{
    ...
    if (gs_parseXml(xml_data, xml_data_size, "<error>", 7, error, 512) > 0) {
        ...
        if (gs_parseXml(xml_data, xml_data_size, "<member>", 8, output, 512) > 0) {
        ...
        if (gs_parseXml(xml_data, xml_data_size, "<admin_count>", 13, output, 512) > 0) {
            group.admins = atoi(output);
        }
        else {
            group.admins = -1;
        }
        if (gs_parseXml(xml_data, xml_data_size, "<member_count>", 14, output, 512) > 0) {
